...
func TestApply_Post(t *testing.T) {
	...
	assert.True(t, ok2)
	assert.Equal(t, application.TargetContent{Id:1, Role:"normal", Name:"role1", AppName:"app1"}, *b[0].TargetContent)

}


func createTestUserAndTargets(th *TestHelper) iuser.User {
	mctx := getModelContext(th.Ctx)
	admin := createTestUserWithEmail(th, "testadmin")
	u := createTestUserWithEmail(th, "testuser")
	th.loginWithScope("testadmin", "write:app write:group write:role")
	createApp(th)
	createRootRole(th)
...
func callPostApplicationOfGroup(th *TestHelper) (int, interface{}) {
	r, _ := http.NewRequest("POST", "http://sso.example.com/api/applications",
		strings.NewReader(`{"target_type": "group", "reason":"testing", "target": [{"id": 2,"role":"normal"}, {"id": 3,"role":"normal"}]}`))
	return ApplicationsResource{}.Post(th.Ctx, r)
}

func callPostApplicationOfRole(th *TestHelper) (int, interface{}) {
	r, _ := http.NewRequest("POST", "http://sso.example.com/api/applications",
		strings.NewReader(`{"target_type": "role", "reason":"testing", "target": [{"id": 1,"role":"normal"}]}`))
	return ApplicationsResource{}.Post(th.Ctx, r)
}
...
func callGetApplication(th *TestHelper) (int, interface{}) {
	...
	return ApplicationsResource{}.Get(th.Ctx, r)
}
...
func TestApplicationHandle_Post(t *testing.T) {
	...
	user := createTestUserAndTargets(th)
	th.login("testuser")
	callPostApplicationOfGroup(th)
	th.logout()
	th.login("testadmin")
...
func callPostApplicationHandle (th *TestHelper, id string, action string) (int, interface{}) {
	...
	aMock := mockParams(th, map[string]string{
		"id": id})
	defer aMock.restore()
	return ApplicationResource{}.Put(th.Ctx, r)
}