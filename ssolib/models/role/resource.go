...
func UpdateResources(ctx *models.Context, resources []Resource) error {
	...
	for _, resource := range resources {
		_, err := tx.Exec(
			"UPDATE resource SET name=?, fullname=?, data=? WHERE id=?", resource.Name, resource.Description,resource.Data,resource.Id)
		if err != nil {
			tx.Rollback()
			return err
		}
...
func DeleteResources(ctx *models.Context, ids []int) error {
	tx := ctx.DB.MustBegin()
	query, args, err := sqlx.In("DELETE FROM role_resource WHERE resource_id IN(?)", ids)
	if err != nil {
		tx.Rollback()
		return err
	}
	...
	query, args, err = sqlx.In("DELETE FROM resource WHERE id IN(?)", ids)
	if err != nil {
		tx.Rollback()
		return err
	}
