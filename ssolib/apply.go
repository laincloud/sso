...
func applyEnterGroup(mctx *models.Context, id int, target application.TargetContent, email string, reason string)(*application.Application, error) {
	res, err := application.CreateApplication(mctx, email, "group", &target, reason)
	if err != nil {
		return nil, err
	}
