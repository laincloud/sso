...
func TestAppResource_Delete(t *testing.T) {
	th := NewTestHelper(t)
	createTestUserWithEmail(th, "testuser")
	...
	a, ok := resp.(string)
	assert.True(t, ok)
	assert.Equal(t, "app doesn't exist", a)
}
...
func deleteApp(th *TestHelper) (int, interface{}){
	r, _ := http.NewRequest("DELETE", "http://sso.example.com/api/app/1",
		nil)
	aMock := mockParams(th, map[string]string{
		"id":   "1"})
	defer aMock.restore()
	return AppResource{}.Delete(th.Ctx, r)
}
