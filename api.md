...
### 3.4 删除资源
**接口地址及请求方式**  
`DELETE /api/resources/:id?app_id=123` 
...
字段名 |变量名 | 是否必填 | 类型 | 示例 | 描述 |
---| --- | --- | --- | --- | --- | 
无|资源id数组|是|array|[1,2,3,4]|body中只有这个数组
...
note:如果all=true，返回app下所有的roles，不返回role成员
**返回结果**
...
字段名 |变量名 | 是否必填 | 类型 | 示例 | 描述 |
---| --- | --- | --- | --- | --- | 
action|操作|是|string|delete update||
...
target|申请目标|是|[]struct{name, role}|[{name:group1,role:admin},{name:group2,role:normal}]||
